...
PER_CAPTURE = 0.05  # time in seconds to wait between captures
...
print(
	f"📍Initial Current location → lat: {lat}, lon: {lon}, actual alt: {alt} compensated alt: {alt_compensation}"
)

gz.takeoff(master, target_altitude=takeoff_altitude)
print(
	f"📍 Takeoff {takeoff_altitude} m location → lat: {lat}, lon: {lon}, actual alt: {takeoff_altitude}"
)
...
for idx, (lat, lon, alt) in enumerate(waypoints):
	...
	gz.goto_waypoint(
		master, lat, lon, alt, timeout=100, alt_compensation=alt_compensation
	)
	print(
		f"[MAVLINK]  → flying to waypoint {idx} at lat: {lat}, lon: {lon}, alt: {alt}"
	)
	...
	while True:
		...
		if not ret:
			print(
				f"[CAMERA]  →   ⚠️ failed to grab frame at waypoint {idx}, skipping slice."
			)
			continue
		...
		if current_lat is None or current_lon is None:
			print(
				f"[MAVLINK]  →   ⚠️ failed to get current GPS location at waypoint {idx}, skipping slice."
			)
			continue
		...
		if coords is None or center_pose is None:
			...
			cv2.waitKey(
				1
			)  # NOTE: this is important to show the frame, otherwise it will block the stream
			continue
